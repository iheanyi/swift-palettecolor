...
The AndroidX quantizer, target scoring and default filter are ported from AndroidX Palette at commit [`81432a7d`](https://github.com/androidx/androidx/tree/81432a7d3a860faea8b18fb2a4712f1d6420f705/palette/palette/src/main/java/androidx/palette/graphics) and are kept integer/`Float` exact, so the same RGB888 pixel vector produces the same swatches on Android and Apple platforms. The HCT/CAM16 solver, Celebi quantizer, `Score` and `TonalPalette` come from [material-color-utilities](https://github.com/material-foundation/material-color-utilities) at commit [`5b3618b`](https://github.com/material-foundation/material-color-utilities/tree/5b3618b16fdc3825e21d5679bafd144662088ea1); the quantizers and `Score` are ported from its Java implementation, the one Android uses, so the seed matches Android bit for bit (see [Vendored Material Color Utilities](#vendored-material-color-utilities)).
...
`Sources/PaletteColor/MaterialColorUtilities/` holds the HCT/CAM16 solver, `QuantizerCelebi` (Wu + Wsmeans), `Score` and `TonalPalette` from [material-color-utilities](https://github.com/material-foundation/material-color-utilities) at commit `5b3618b`. It is vendored rather than declared as a dependency because the Swift package lives in a subdirectory of an untagged monorepo, which SwiftPM cannot reference. The copy is kept `internal` so an app that also depends on `MaterialColorUtilities` directly gets no duplicate symbols; `HCT` and `MediaArtworkScheme` are the public surface.

The HCT, tonal palette and color math are the official Swift port with access control reduced. The quantizers and `Score` are ported from the **Java** implementation instead, because that is what Android (and SystemUI) run and the Swift port, which follows the Dart version, disagrees with it: it iterates 5 times instead of 10, has no minimum-movement rule, seeds extra centroids with a generator that reaches only half of any range (a loop that never ends on artwork whose Wu pass returns few clusters), sorts Wu's clusters, divides a variance as integers, and reads colors from an unordered `Dictionary`. The port reproduces Java's `int` wrap-around, `LinkedHashMap` order, stable `Double.compare` sort and `java.util.Random(0x42688)`, and is checked against the Java code on real cover art and synthetic edge cases (`MaterialColorUtilitiesParityTests`). Every loop is bounded. See [NOTICE](NOTICE).
